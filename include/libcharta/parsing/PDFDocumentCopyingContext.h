...
class PDFDocumentCopyingContext
{
  public:
    ...
    /* This one is a bit tricky:
        ...
        YOU ARE REQUIRED TO COPY THEM IMMEDIATELY LATER using CopyNewObjectsForDirectObject...otherwise the PDF will be
       fault. The reason for not writing them at point of writing the direct object is simple - 100% of your usages of
       this method it will be in the context of writing something encapsulating this code...you don't want to be
       disturbed. still...there might be referenced objects from this object to copy, hence you must later copy them.
    ...
    */
    ...
    /* Use this method to signal the copying context to avoid copying some objects, but rather use replacements.
        The input map has as keys source object IDs (you can scan for these objects earlier using the parser or some
       other methods. The values in this map are object IDs to use instead. these should be valid IDs in the target PDF,
       though they do not have to be defined yet (can be forward declaration. Note that any keys that are mapped to
       objects already copied, will be ignored, rather than replaced.
    */
    void ReplaceSourceObjects(const ObjectIDTypeToObjectIDTypeMap &inSourceObjectsToNewTargetObjects);

    // Used directly, the extendedrs used here will only get CreateFormXObjectsFromPDF and AppendPDFPagesFromPDF events
    // for this copying context. to get full extension of the documetn context, make sure to add the listeners through
