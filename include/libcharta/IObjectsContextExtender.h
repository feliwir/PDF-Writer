...
class IObjectsContextExtender
{
  ...
  public:
    ...
    virtual charta::IByteWriter *GetCompressionWriteStream(charta::IByteWriterWithPosition *inOutputStream) = 0;
    ...
    // the implementation should finalize the compression stream write [any remaning footer buffers should be written
    // now]. this would allow the PDFStream to calculate the extent on the actual write stream (given as input for
    // GetCompressionWriteStream).
    virtual void FinalizeCompressedStreamWrite(charta::IByteWriter *inCompressedStream) = 0;
};