...
void AbstractContentContext::c(double inX1, double inY1, double inX2, double inY2, double inX3, double inY3)
{
    RenewStreamConnection();
    AssertProcsetAvailable(KProcsetPDF);
...
void AbstractContentContext::v(double inX2, double inY2, double inX3, double inY3)
{
    RenewStreamConnection();
    AssertProcsetAvailable(KProcsetPDF);
...
void AbstractContentContext::y(double inX1, double inY1, double inX3, double inY3)
{
    RenewStreamConnection();
    AssertProcsetAvailable(KProcsetPDF);
...
void AbstractContentContext::DoubleQuoteLow(double inWordSpacing, double inCharacterSpacing, const std::string &inText)
{
    RenewStreamConnection();
    AssertProcsetAvailable(KProcsetPDF);
    AssertProcsetAvailable(KProcsetText);
...
void AbstractContentContext::DoubleQuoteHexLow(double inWordSpacing, double inCharacterSpacing,
...
void AbstractContentContext::TJLow(const std::list<StringOrDouble> &inStringsAndSpacing)
{
    RenewStreamConnection();
    AssertProcsetAvailable(KProcsetPDF);
    AssertProcsetAvailable(KProcsetText);
    ...
    for (const auto &stringOrSpacing : inStringsAndSpacing)
    {
        if (std::holds_alternative<double>(stringOrSpacing))
            mPrimitiveWriter.WriteDouble(std::get<double>(stringOrSpacing));
        else
            mPrimitiveWriter.WriteLiteralString(std::get<std::string>(stringOrSpacing));
    }
...
void AbstractContentContext::TJHexLow(const std::list<StringOrDouble> &inStringsAndSpacing)
{
    RenewStreamConnection();
    AssertProcsetAvailable(KProcsetPDF);
    AssertProcsetAvailable(KProcsetText);
    ...
    for (const auto &stringOrSpacing : inStringsAndSpacing)
    {
        if (std::holds_alternative<double>(stringOrSpacing))
            ...
            mPrimitiveWriter.WriteHexString(std::get<std::string>(stringOrSpacing));
    }
...
EStatusCode AbstractContentContext::WriteTextCommandWithEncoding(const std::string &inUnicodeText,
...
{
  public:
    TjCommand(AbstractContentContext *inContext)
    {
        mContext = inContext;
    }
...
{
  public:
    QuoteCommand(AbstractContentContext *inContext)
    {
        mContext = inContext;
    }
...
{
  public:
    DoubleQuoteCommand(AbstractContentContext *inContext, double inWordSpacing, double inCharacterSpacing)
    {
        mContext = inContext;
        mWordSpacing = inWordSpacing;
...
EStatusCode AbstractContentContext::DoubleQuote(double inWordSpacing, double inCharacterSpacing,
                                                const std::string &inText)
{
...
EStatusCode AbstractContentContext::TJ(const std::list<StringOrDouble> &inStringsAndSpacing)
{
    PDFUsedFont *currentFont = mGraphicStack.GetCurrentState().mFont;
    if (currentFont == nullptr)
    {
    ...
    for (const auto &stringOrSpacing : inStringsAndSpacing)
    {
        if (std::holds_alternative<double>(stringOrSpacing))
        {
            parameters.emplace_back(std::get<double>(stringOrSpacing));
        }
        else
        {
            GlyphUnicodeMappingList glyphsAndUnicode;
...
EStatusCode AbstractContentContext::WriteTextCommandWithDirectGlyphSelection(const GlyphUnicodeMappingList &inText,
                                                                             ITextCommand *inTextCommand)
{
    PDFUsedFont *currentFont = mGraphicStack.GetCurrentState().mFont;
    if (currentFont == nullptr)
...
EStatusCode AbstractContentContext::TJ(const std::list<GlyphUnicodeMappingListOrDouble> &inStringsAndSpacing)
{
    PDFUsedFont *currentFont = mGraphicStack.GetCurrentState().mFont;
    if (currentFont == nullptr)
    {
    ...
    std::list<GlyphUnicodeMappingList> stringsList;

    for (const auto &stringOrSpacing : inStringsAndSpacing)
        if (std::holds_alternative<GlyphUnicodeMappingList>(stringOrSpacing))
            stringsList.push_back(std::get<GlyphUnicodeMappingList>(stringOrSpacing));
    ...
    if (writeAsCID)
    {
        for (const auto &stringOrSpacing : inStringsAndSpacing)
        {
            if (std::holds_alternative<double>(stringOrSpacing))
            {
                stringOrDoubleList.emplace_back(std::get<double>(stringOrSpacing));
            }
            else
            {
...
void AbstractContentContext::DrawRectangle(double inLeft, double inBottom, double inWidth, double inHeight,
                                           const GraphicOptions &inOptions)
{
...
void AbstractContentContext::DrawCircle(double inCenterX, double inCenterY, double inRadius,
                                        const GraphicOptions &inOptions)
{
...
void AbstractContentContext::SetupColor(EDrawingType inDrawingType, unsigned long inColorValue,
                                        EColorSpace inColorSpace)
{
    if (inDrawingType != eStroke && inDrawingType != eFill)
        return;
...
void AbstractContentContext::DrawImage(double inX, double inY, const std::string &inImagePath,
                                       const ImageOptions &inOptions)
{
    ...
    else if (inOptions.transformationMethod == eFit)
    {
        auto imageDimensions =
            mDocumentContext->GetImageDimensions(inImagePath, inOptions.imageIndex, inOptions.pdfParsingOptions);
        ...
        else if (imageDimensions.first > inOptions.boundingBoxWidth ||
                 imageDimensions.second > inOptions.boundingBoxHeight) // overflow
        {
            scaleX = imageDimensions.first > inOptions.boundingBoxWidth
                         ? inOptions.boundingBoxWidth / imageDimensions.first
                         : 1;
            scaleY = imageDimensions.second > inOptions.boundingBoxHeight
                         ? inOptions.boundingBoxHeight / imageDimensions.second
                         : 1;
        }
    ...
    q();
    cm(transformation[0], transformation[1], transformation[2], transformation[3], transformation[4],
       transformation[5]);
