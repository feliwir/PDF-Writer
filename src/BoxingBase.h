...
template <typename T> class BoxingBase
{
  public:
    BoxingBase();
...
template <typename T> BoxingBase<T>::BoxingBase()
{
    // maybe do null assignment sometime
}

template <typename T> BoxingBase<T>::BoxingBase(const T &inValue)
{
    boxedValue = inValue;
}

template <typename T> BoxingBase<T>::BoxingBase(const BoxingBase<T> &inOther)
{
    boxedValue = inOther.boxedValue;
}

template <typename T> BoxingBase<T> &BoxingBase<T>::operator=(const T &inValue)
{
    boxedValue = inValue;
    return *this;
}

template <typename T> BoxingBase<T>::operator T() const
{
    return boxedValue;
}

template <typename T> class STDStreamsReader
{
  public:
...
template <typename T> void STDStreamsReader<T>::Read(const std::string &inReadFrom, T &outValue)
{
    std::stringstream stream(inReadFrom);
    stream >> outValue;
}

template <typename T> void STDStreamsReader<T>::Read(const std::wstring &inReadFrom, T &outValue)
{
    std::wstringstream stream(inReadFrom);
    stream >> outValue;
}

template <typename T> class STDStreamsWriter
{
  public:
...
template <typename T> void STDStreamsWriter<T>::Write(const T &inValue, std::string &outWriteTo)
{
...
template <typename T> void STDStreamsWriter<T>::Write(const T &inValue, std::wstring &outWriteTo)
{
...
template <typename U, class Reader, class Writer>
BoxingBaseWithRW<U, Reader, Writer>::BoxingBaseWithRW(const BoxingBaseWithRW<U, Reader, Writer> &inOther)
    : BoxingBase<U>(inOther)
{
}
...
template <typename U, class Reader, class Writer> std::string BoxingBaseWithRW<U, Reader, Writer>::ToString() const
{
    Writer writer;
    std::string aString;
...
template <typename U, class Reader, class Writer> std::wstring BoxingBaseWithRW<U, Reader, Writer>::ToWString() const
{
    Writer writer;
    std::wstring aString;
