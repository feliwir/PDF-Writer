...
EStatusCode ANSIFontWriter::WriteFont(FreeTypeFaceWrapper &inFontInfo, WrittenFontRepresentation *inFontOccurrence,
...
void ANSIFontWriter::CalculateDifferences()
{
    ...
    for (; it != mCharactersVector.end(); ++it)
    {
        ...
        if (strcmp(glyphName.c_str(),
                   winAnsiEncoding.GetEncodedGlyphName((IOBasicTypes::Byte)it->second.mEncodedCharacter)) != 0)
...
void ANSIFontWriter::WriteEncodingDictionary()
{
    ...
    for (; it != mDifferences.end(); ++it)
    {
        if (previousEncoding + 1 != it->first)
        {
            ...
            // put two spaces to account for the starting bracket and following spaces. y'know, just for the sake of
            // proper formatting
...
static const char *scCmapHeader = "/CIDInit /ProcSet findresource begin\n\
12 dict begin\n\
begincmap\n\
/CIDSystemInfo\n\
<< /Registry (Adobe)\n\
