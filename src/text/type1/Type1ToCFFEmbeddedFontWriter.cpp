...
// NOTE - this is not the original array order. it's a sorted one. i'm using it to determine whether and at what
// position a string might be in the standard strings array.
static const char *scSortedStandardStrings[N_STD_STRINGS] = {".notdef",
                                                             "001.000",
                                                             "001.001",
                                                             "001.002",
                                                             "001.003",
...
static uint16_t scSortedStandardStringsPositions[N_STD_STRINGS] = {
    ...
    43,  283, 44,  284, 45,  386, 140, 306, 285, 46,  313, 387, 286, 47,  287, 186, 364, 48,  142, 370, 187, 366, 188,
    367, 189, 369, 316, 190, 365, 141, 371, 288, 191, 368, 49,  289, 50,  290, 51,  388, 317, 389, 291, 52,  192, 307,
    390, 292, 53,  157, 377, 303, 293, 54,  193, 373, 194, 374, 195, 375, 196, 372, 294, 55,  295, 56,  296, 57,  297,
    58,  197, 376, 198, 378, 298, 59,  199, 308, 299, 66,  200, 201, 125, 202, 144, 203, 7,   233, 204, 63,  95,  11,
    253, 33,  205, 67,  61,  93,  92,  94,  60,  62,  129, 160, 254, 116, 68,  136, 206, 133, 97,  343, 305, 255, 126,
    27,  300, 13,  346, 249, 170, 103, 69,  112, 113, 161, 131, 159, 5,   344, 231, 232, 130, 145, 256, 70,  207, 208,
    209, 210, 25,  341, 247, 331, 121, 137, 111, 30,  257, 167, 2,   96,  304, 229, 71,  266, 267, 268, 109, 314, 22,
    322, 338, 244, 328, 110, 101, 21,  337, 243, 327, 99,  72,  149, 124, 31,  106, 120, 107, 108, 73,  134, 14,  315,
    272, 74,  211, 212, 213, 214, 258, 75,  76,  77,  29,  151, 146, 259, 78,  128, 166, 260, 152, 168, 79,  26,  342,
    248, 332, 261, 215, 4,   80,  216, 217, 218, 148, 135, 219, 18,  238, 320, 301, 155, 334, 240, 158, 150, 324, 139,
    143, 147, 262, 220, 81,  115, 9,   269, 235, 10,  270, 236, 6,   15,  114, 345, 251, 122, 12,  156, 82,  32,  123,
    319, 252, 3,   118, 105, 119, 65,  8,   117, 104, 83,  165, 132, 263, 302, 84,  221, 102, 28,  24,  323, 340, 246,
    330, 23,  339, 245, 329, 16,  1,   264, 98,  85,  162, 20,  321, 336, 242, 163, 250, 169, 127, 153, 265, 19,  237,
    335, 241, 164, 325, 86,  222, 223, 224, 225, 64,  87,  88,  89,  90,  226, 227, 100, 91,  228, 17,  333, 239, 326};
...
EStatusCode Type1ToCFFEmbeddedFontWriter::CreateCFFSubset(FreeTypeFaceWrapper &inFontInfo,
                                                          ...
                                                          MyStringBuf &outFontProgram)
{
    ...
    do
    {
        ...
        {
            if (!FSType(mType1Input.mFontInfoDictionary.FSTypeValid ? mType1Input.mFontInfoDictionary.fsType
        ...
        // Found big gap between FreeType indexing and the way it's in the Type 1. obvioulsy due to encoding
        // differences. So i'm replacing the indexes of free type, with names...should be safer (also cleans up invalid
        // glyph ids, in case direct glyphs placement put them here)
...
EStatusCode Type1ToCFFEmbeddedFontWriter::AddComponentGlyphs(const std::string &inGlyphID, StringSet &ioComponents,
                                                             bool &outFoundComponents)
{
    ...
    if (PDFHummus::eSuccess == status && !dependencies.mCharCodes.empty())
    {
        ...
        for (; it != dependencies.mCharCodes.end() && PDFHummus::eSuccess == status; ++it)
        {
            bool dummyFound;
            /*
                Using standard encoding instead of the font encoding, because SEAC (the only operator to create glyph
               dependency in type 1) relies on standard encoding indexes by definition.
            */
...
EStatusCode Type1ToCFFEmbeddedFontWriter::WriteTopDictSegment(MyStringBuf &ioTopDictSegment)
{
    ...
    AddNumberOperandIfNotDefault(dictPrimitiveWriter, (int)(mType1Input.mFontInfoDictionary.isFixedPitch ? 1 : 0),
                                 0xC01, 0);
    ...
    // FontMatrix
    if (mType1Input.mFontDictionary.FontMatrix[0] != 0.001 || mType1Input.mFontDictionary.FontMatrix[1] != 0 ||
        mType1Input.mFontDictionary.FontMatrix[2] != 0 || mType1Input.mFontDictionary.FontMatrix[3] != 0.001 ||
        mType1Input.mFontDictionary.FontMatrix[4] != 0 || mType1Input.mFontDictionary.FontMatrix[5] != 0)
    {
    ...
    // FontBBox
    if (mType1Input.mFontDictionary.FontBBox[0] != 0 || mType1Input.mFontDictionary.FontBBox[1] != 0 ||
        mType1Input.mFontDictionary.FontBBox[2] != 0 || mType1Input.mFontDictionary.FontBBox[3] != 0)
    {
    ...
    if (mType1Input.mFontDictionary.FSTypeValid || mType1Input.mFontInfoDictionary.FSTypeValid)
    {
        ...
        formatter << "/FSType "
                  << (mType1Input.mFontInfoDictionary.FSTypeValid ? mType1Input.mFontInfoDictionary.fsType
                                                                  : mType1Input.mFontDictionary.fsType)
                  << " def";
        dictPrimitiveWriter.WriteIntegerOperand(AddStringToStringsArray(formatter.str()));
