...
FreeTypeOpenTypeWrapper::~FreeTypeOpenTypeWrapper() = default;
...
{
    // For TTF that's a good calculation. For CFFs i should be able to ge this from the private dict information, like
    // the case for type 1 (just use StdVW). for this i'll have to parse the CFF...differ for now instead use the method
    // for true type which would be to try and measure lower case l, and if unsuccesful use some heuristic
