...
EStatusCode CFFANSIFontWriter::WriteFont(FreeTypeFaceWrapper &inFontInfo, WrittenFontRepresentation *inFontOccurrence,
                                         ObjectsContext *inObjectsContext, bool inEmbedFont)
{
    ...
    if (inEmbedFont)
    {
        ...
        if (strcmp(scType1Type, fontType) == 0)
        {
            ...
            status =
                embeddedFontWriter.WriteEmbeddedFont(inFontInfo, inFontOccurrence->GetGlyphIDsAsOrderedVector(),
                                                     scType1C, fontName, inObjectsContext, mEmbeddedFontFileObjectID);
        }
...
// static const uint8_t scLeftParanthesis[] = {'('};
// static const uint8_t scRightParanthesis[] = {')'};
