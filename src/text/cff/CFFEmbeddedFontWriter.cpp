...
EStatusCode CFFEmbeddedFontWriter::WriteStringIndex()
{
    ...
    if (mOptionalEmbeddedPostscript.size() == 0)
    {
        ...
        return streamCopier.CopyToOutputStream(
            ...
            (LongBufferSizeType)(mOpenTypeInput.mCFF.mGlobalSubrsPosition - mOpenTypeInput.mCFF.mStringIndexPosition));
    }
    else
    {
        ...
        for (int i = 0; i < mOpenTypeInput.mCFF.mStringsCount; ++i)
        {
            mFontFileStream.Write((const Byte *)(mOpenTypeInput.mCFF.mStrings[i]),
                                                ...
                                                const FontDictInfoToByteMap &inNewFontDictsIndexes)
{
    ...
    do
    {
        ...
        for (itFontInfos = inNewFontDictsIndexes.begin();
             itFontInfos != inNewFontDictsIndexes.end() && PDFHummus::eSuccess == status; ++itFontInfos, ++i)
        {
