...
EStatusCode PDFWriter::StartPDF(const std::string &inOutputFilePath, EPDFVersion inPDFVersion,
                                ...
                                const PDFCreationSettings &inPDFCreationSettings)
{
    ...
    {
        mDocumentContext.SetupEncryption(inPDFCreationSettings.DocumentEncryptionOptions,
                                         thisOrDefaultVersion(inPDFVersion));
...
void PDFWriter::SetupLog(const LogConfiguration &inLogConfiguration)
{
    if (inLogConfiguration.LogStream != nullptr)
        ...
        Trace::DefaultTrace().SetLogSettings(inLogConfiguration.LogFileLocation, inLogConfiguration.ShouldLog,
                                             inLogConfiguration.StartWithBOM);
}
...
PDFFormXObject *PDFWriter::StartFormXObject(const PDFRectangle &inBoundingBox, ObjectIDType inFormXObjectID,
...
{
    return mDocumentContext.CreateFormXObjectsFromPDF(inPDFFilePath, inParsingOptions, inPageRange,
...
EStatusCodeAndObjectIDTypeList PDFWriter::CreateFormXObjectsFromPDF(const std::string &inPDFFilePath,
                                                                    ...
                                                                    const PDFParsingOptions &inParsingOptions)
{
    return mDocumentContext.CreateFormXObjectsFromPDF(inPDFFilePath, inParsingOptions, inPageRange, inCropBox,
...
EStatusCodeAndObjectIDTypeList PDFWriter::AppendPDFPagesFromPDF(const std::string &inPDFFilePath,
                                                                ...
                                                                const PDFParsingOptions &inParsingOptions)
{
    return mDocumentContext.AppendPDFPagesFromPDF(inPDFFilePath, inParsingOptions, inPageRange,
                                                  inCopyAdditionalObjects);
}
...
EStatusCode PDFWriter::ContinuePDF(const std::string &inOutputFilePath, const std::string &inStateFilePath,
    ...
    do
    {
        ...
        if (mIsModified)
        {
            PDFObjectCastPtr<PDFInteger> isModifiedFileVersionObject(
                pdfWriterDictionary->QueryDirectObject("mModifiedFileVersion"));
...
EStatusCode PDFWriter::MergePDFPagesToPage(PDFPage &inPage, const std::string &inPDFFilePath,
                                           ...
                                           const PDFParsingOptions &inParsingOptions)
{
    return mDocumentContext.MergePDFPagesToPage(inPage, inPDFFilePath, inParsingOptions, inPageRange,
                                                inCopyAdditionalObjects);
}
...
EStatusCodeAndObjectIDTypeList PDFWriter::CreateFormXObjectsFromPDF(IByteReaderWithPosition *inPDFStream,
                                                                    ...
                                                                    const PDFParsingOptions &inParsingOptions)
{
    return mDocumentContext.CreateFormXObjectsFromPDF(inPDFStream, inParsingOptions, inPageRange,
                                                      inPageBoxToUseAsFormBox, inTransformationMatrix,
                                                      inCopyAdditionalObjects);
}
...
EStatusCodeAndObjectIDTypeList PDFWriter::CreateFormXObjectsFromPDF(IByteReaderWithPosition *inPDFStream,
                                                                    ...
                                                                    const PDFParsingOptions &inParsingOptions)
{
    return mDocumentContext.CreateFormXObjectsFromPDF(inPDFStream, inParsingOptions, inPageRange, inCropBox,
                                                      inTransformationMatrix, inCopyAdditionalObjects);
}
...
EStatusCode PDFWriter::MergePDFPagesToPage(PDFPage &inPage, IByteReaderWithPosition *inPDFStream,
                                           ...
                                           const PDFParsingOptions &inParsingOptions)
{
    return mDocumentContext.MergePDFPagesToPage(inPage, inPDFStream, inParsingOptions, inPageRange,
                                                inCopyAdditionalObjects);
}
...
EStatusCode PDFWriter::ModifyPDF(const std::string &inModifiedFile, EPDFVersion inPDFVersion,
                                          ...
                                          const PDFCreationSettings &inPDFCreationSettings)
{
    ...
    return SetupStateFromModifiedStream(inModifiedSourceStream, thisOrDefaultVersion(inPDFVersion),
                                                  ...
                                                  const PDFCreationSettings &inPDFCreationSettings)
{
    ...
    do
    {
        ...
        status = SetupStateFromModifiedStream(mModifiedFile.GetInputStream(), thisOrDefaultVersion(inPDFVersion),
                                              inPDFCreationSettings);
    } while (false);
                                             ...
                                             EPDFVersion inOveridePDFVersion)
{
    ...
    return PDFWriter::RecryptPDF(originalPDF.GetInputStream(), inOriginalPDFPassword, newPDF.GetOutputStream(),
                                             ...
                                             EPDFVersion inOveridePDFVersion)
{
    ...
    */
    ...
    do
    {
        // open PDF copying context for the source document (before starting a new one, so i can get the origina level
        // and set the same)
        copyingContext =
        ...
        status = pdfWriter.StartPDFForStream(
            inNewPDFStream,
            (ePDFVersionUndefined == inOveridePDFVersion)
