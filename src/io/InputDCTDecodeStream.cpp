...
EStatusCode InputDCTDecodeStream::StartRead()
{
    ...
    try
    {
        ...
        mSamplesBuffer = (*mJPGState.mem->alloc_sarray)((j_common_ptr)&mJPGState, JPOOL_IMAGE, row_stride,
                                                        mJPGState.rec_outbuf_height);
        mCurrentSampleRow = 0;
        mTotalSampleRows = 0;
        mIndexInRow = 0;
...
uint8_t *InputDCTDecodeStream::CopySamplesArrayToBuffer(uint8_t *inBuffer, size_t inBufferSize)
{
    ...
    while (mCurrentSampleRow < mTotalSampleRows && ((size_t)(indexInBuffer - inBuffer)) < inBufferSize)
    {
        if ((inBufferSize - (indexInBuffer - inBuffer)) < (row_stride - mIndexInRow))
        {
            ...
            memcpy(indexInBuffer, mSamplesBuffer[mCurrentSampleRow] + mIndexInRow,
                   inBufferSize - (indexInBuffer - inBuffer));
