...
void TIFFImageHandler::InitializeConversionState()
{
    ...
    // if does not have a compression that is either zip or ccit, this should cause it to compress in zip...like all
    // other normal compression
...
EStatusCode TIFFImageHandler::ReadTopLevelTiffInformation()
{
    ...
    do
    {
        ...
        for (i = 0; i < mT2p->tiff_pagecount; i++)
        {
            ...
            if (TIFFGetField(mT2p->input, TIFFTAG_ICCPROFILE, &(mT2p->tiff_iccprofilelength),
                             &(mT2p->tiff_iccprofile)) != 0)
            {
                mT2p->tiff_pages[i].page_extra++;
            }
            mT2p->tiff_tiles[i].tiles_tilecount = mT2p->tiff_pages[i].page_tilecount;
            if ((TIFFGetField(mT2p->input, TIFFTAG_PLANARCONFIG, &xuint16) != 0) && (xuint16 == PLANARCONFIG_SEPARATE))
            {
            ...
            {
                TIFFGetField(mT2p->input, TIFFTAG_TILEWIDTH, &(mT2p->tiff_tiles[i].tiles_tilewidth));
...
EStatusCode TIFFImageHandler::ReadTIFFPageInformation() // t2p_read_tiff_data
{
    ...
    do
    {
        ...
        if (TIFFGetField(mT2p->input, TIFFTAG_ICCPROFILE, &(mT2p->tiff_iccprofilelength), &(mT2p->tiff_iccprofile)) !=
            0)
        {
...
void TIFFImageHandler::ComposePDFPage()
{
    ...
    {
        mT2p->pdf_imagewidth = ((float)(mT2p->tiff_width)) * PS_UNIT_SIZE / mT2p->pdf_xres;
        mT2p->pdf_imagelength = ((float)(mT2p->tiff_length)) * PS_UNIT_SIZE / mT2p->pdf_yres;
    }
...
PDFImageXObject *TIFFImageHandler::WriteTileImageXObject(int inTileIndex)
{
    ...
    do
    {
        ...
        imageContext->WriteIntegerValue(TileIsRightEdge(inTileIndex)
        ...
        imageContext->WriteIntegerValue(TileIsBottomEdge(inTileIndex)
...
void TIFFImageHandler::WriteImageXObjectFilter(DictionaryContext *inImageDictionary, int inTileIndex)
{
    ...
    case T2P_COMPRESS_G4:
        ...
        else
        {
            // Columns
            decodeParmsDictionary->WriteKey(scColumns);
            decodeParmsDictionary->WriteIntegerValue(TileIsRightEdge(inTileIndex)
                                                         ? mT2p->tiff_tiles[mT2p->pdf_page].tiles_edgetilewidth
                                                         : mT2p->tiff_tiles[mT2p->pdf_page].tiles_tilewidth);

            // Rows
            decodeParmsDictionary->WriteKey(scRows);
            decodeParmsDictionary->WriteIntegerValue(TileIsBottomEdge(inTileIndex)
                                                         ? mT2p->tiff_tiles[mT2p->pdf_page].tiles_edgetilelength
                                                         : mT2p->tiff_tiles[mT2p->pdf_page].tiles_tilelength);
        }
...
static tsize_t t2p_readproc(thandle_t handle, tdata_t data, tsize_t size)
{
...
static toff_t t2p_seekproc(thandle_t handle, toff_t offset, int whence)
{
...
static toff_t t2p_sizeproc(thandle_t handle)
{
    (void)handle;
    return -1;
}

static int t2p_mapproc(thandle_t handle, tdata_t *data, toff_t *offset)
{
    (void)handle, (void)data, (void)offset;
    return -1;
}

static void t2p_unmapproc(thandle_t handle, tdata_t data, toff_t offset)
{
    (void)handle, (void)data, (void)offset;
}
    ...
    do
    {
        ...
        else
        {
            ...
            if (mT2p->pdf_sample == T2P_SAMPLE_PLANAR_SEPARATE_TO_CONTIG)
            {
                ...
                {
                    read = TIFFReadEncodedTile(mT2p->input, inTileIndex + i * tilecount,
        ...
        status = WriteImageBufferToStream(
            inImageStream,
            TileIsRightEdge(inTileIndex) ? mT2p->tiff_tiles[mT2p->pdf_page].tiles_edgetilewidth
                                         : mT2p->tiff_tiles[mT2p->pdf_page].tiles_tilewidth,
            TileIsBottomEdge(inTileIndex) ? mT2p->tiff_tiles[mT2p->pdf_page].tiles_edgetilelength
                                          : mT2p->tiff_tiles[mT2p->pdf_page].tiles_edgetilelength,
...
tsize_t TIFFImageHandler::SampleLABSignedToUnsigned(tdata_t inBuffer, uint32 inSampleCount)
{
    ...
    for (i = 0; i < inSampleCount; i++)
    {
        ...
        {
            ((unsigned char *)inBuffer)[(i * 3) + 1] = (unsigned char)(0x80 + ((char *)inBuffer)[(i * 3) + 1]);
        }
        else
        {
        ...
        {
            ((unsigned char *)inBuffer)[(i * 3) + 2] = (unsigned char)(0x80 + ((char *)inBuffer)[(i * 3) + 2]);
        }
        else
        {
                                        ...
                                        uint32 inEdgeTileWidth, uint32 inTileLength)
{
    ...
    {
        _TIFFmemcpy(&(((char *)inBuffer)[edgescanwidth * i]), &(((char *)inBuffer)[inScanWidth * i]), edgescanwidth);
    }
}
...
void TIFFImageHandler::WriteCommonImageDictionaryProperties(DictionaryContext *inImageContext)
{
    ...
    if (mUserParameters.BWTreatment.AsImageMask && mT2p->pdf_colorspace == T2P_CS_BILEVEL)
    {
    ...
    // decode
    if ((mT2p->pdf_switchdecode != 0) &&
        !(mT2p->pdf_colorspace == T2P_CS_BILEVEL && mT2p->pdf_compression == T2P_COMPRESS_G4))
    {
...
EStatusCode TIFFImageHandler::WriteImageData(std::shared_ptr<PDFStream> inImageStream)
{
    ...
    do
    {
        ...
        if (mT2p->pdf_sample == T2P_SAMPLE_NOTHING)
        {
            ...
            {
                read = TIFFReadEncodedStrip(mT2p->input, i, (tdata_t)&buffer[bufferoffset], stripsize);
                if (read == -1)
                {
        ...
        else
        {
            if ((mT2p->pdf_sample & T2P_SAMPLE_PLANAR_SEPARATE_TO_CONTIG) != 0)
            {
                ...
                for (i = 0; i < stripcount; i++)
                {
                    ...
                    for (j = 0; j < mT2p->tiff_samplesperpixel; j++)
                    {
                        read = TIFFReadEncodedStrip(mT2p->input, i + j * stripcount,
                                                    (tdata_t) & (samplebuffer[samplebufferoffset]), sepstripsize);
                        if (read == -1)
                        {
                                                       ...
                                                       ImageSizeProc inBufferSizeFunction)
{
    ...
    do
    {
        ...
        TIFF *output = TIFFClientOpen("dummy.txt", "w", (thandle_t)mT2p, t2p_readproc, t2p_writeproc, t2p_seekproc,
                                      t2p_closeproc, t2p_sizeproc, t2p_mapproc, t2p_unmapproc);
        ...
        case T2P_COMPRESS_ZIP:
            ...
            {
                TIFFSetField(output, TIFFTAG_PREDICTOR, mT2p->pdf_defaultcompressionquality % 100);
            }
            if (mT2p->pdf_defaultcompressionquality / 100 != 0)
            {
                TIFFSetField(output, TIFFTAG_ZIPQUALITY, (mT2p->pdf_defaultcompressionquality / 100));
            }
            break;
        }

        mT2p->pdfStream = inPDFStream;
        mT2p->output = output; // dirty trick so i can use the inBufferSizeFunction function by getting info from output
                               // (sometimes)
                                                         ...
                                                         ObjectIDType inFormXObjectID)
{
    ...
    PDFFormXObject *xobjectForm = mContainerDocumentContext->StartFormXObject(
        PDFRectangle(mT2p->pdf_mediabox.x1, mT2p->pdf_mediabox.y1, mT2p->pdf_mediabox.x2, mT2p->pdf_mediabox.y2),
        inFormXObjectID);
    do
    {
        ...
        if (mT2p->tiff_transferfunctioncount != 0)
        {
            ...
            xobjectContentContext->gs(
                xobjectForm->GetResourcesDictionary().AddExtGStateMapping(mT2p->pdf_transfer_functions_gstate));
        }
        ...
        if (mT2p->tiff_tiles[mT2p->pdf_page].tiles_tilecount > 0)
        {
            for (; it != inImages.end() && i < mT2p->tiff_tiles[mT2p->pdf_page].tiles_tilecount; ++it, ++i)
            {
                ...
                xobjectContentContext->q();
                xobjectContentContext->cm(box.mat[0], box.mat[1], box.mat[3], box.mat[4], box.mat[6], box.mat[7]);
                xobjectContentContext->Do(xobjectForm->GetResourcesDictionary().AddImageXObjectMapping(*it));
                xobjectContentContext->Q();
            }
        }
        else
...
void TIFFImageHandler::WriteIndexedCSForBiLevelColorMap()
{
    ...
    for (unsigned long j = 0; j < colorValuesPerComponent; ++j)
    {
        if (4 == colorComponents)
        {
            SAFE_SPRINTF_4(colorsBuffer, 10, scSingleColorCMYKFormat, (unsigned char)max[0] + (unsigned char)min[0],
                           (unsigned char)max[1] + (unsigned char)min[1], (unsigned char)max[2] + (unsigned char)min[2],
                           (unsigned char)max[3] + (unsigned char)min[3]);
        }
        else
        {
            SAFE_SPRINTF_3(colorsBuffer, 10, scSingleColorRGBFormat, (unsigned char)max[0] + (unsigned char)min[0],
                                                                  ...
                                                                  const TIFFUsageParameters &inTIFFUsageParameters)
{
    ...
    return CreateFormXObjectFromTIFFStream(
        inTIFFStream, mObjectsContext->GetInDirectObjectsRegistry().AllocateNewObjectID(), inTIFFUsageParameters);
}
...
static toff_t STATIC_streamSeek(thandle_t inData, toff_t inOffset, int inDirection)
{
    ...
    return (toff_t)((((StreamWithPos *)inData)->mStream)->GetCurrentPosition() -
