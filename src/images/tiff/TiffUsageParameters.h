...
struct TIFFBiLevelBWColorTreatment
{
    ...
    TIFFBiLevelBWColorTreatment(bool inAsImageMask, const CMYKRGBColor &inOneColor)
    {
        AsImageMask = inAsImageMask;
        OneColor = inOneColor;
    }
...
{
    // AsColorMap - is this image to be drawn are regular grayscale (false), or as a combination between 2 colors, where
    // 0 is one color, 1 is the other and any interim value is intepreted as their combination
    bool AsColorMap;
    ...
    TIFFBiLevelGrayscaleColorTreatment(bool inAsColorMap, const CMYKRGBColor &inOneColor,
                                       const CMYKRGBColor &inZeroColor)
    {
...
struct TIFFUsageParameters
{
    ...
    TIFFUsageParameters()
        : BWTreatment(TIFFBiLevelBWColorTreatment::DefaultTIFFBiLevelBWColorTreatment()),
          GrayscaleTreatment(TIFFBiLevelGrayscaleColorTreatment::DefaultTIFFBiLevelGrayscaleColorTreatment())
    {
        PageIndex = 0;
    }
