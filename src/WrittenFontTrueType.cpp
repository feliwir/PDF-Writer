                                                  ...
                                                  UShortList &outEncodedCharacters)
{
    ...
    for (; it != inGlyphsList.end() && encodingResult.first; ++it)
    {
        ...
        else if (0x2022 == it->mUnicodeValues.front())
        {
            ...
            // However, only code 225 is specifically assigned to the bullet character; other codes are subject to
            // future reassignment.

            // now i don't know if it's related or not...but acrobat isn't happy when i'm using winansi with bullet. and
            // text coming after that bullet may be corrupted. so i'm forcing CID if i hit bullet till i know better.
            encodingResult.first = false;
        }
        else
        {
    ...
    if (encodingResult.first)
    {
        ...
        for (; itGlyphs != inGlyphsList.end(); ++itGlyphs, ++itEncoded)
        {
            if (mANSIRepresentation->mGlyphIDToEncodedChar.find(itGlyphs->mGlyphCode) ==
                mANSIRepresentation->mGlyphIDToEncodedChar.end())
                mANSIRepresentation->mGlyphIDToEncodedChar.insert(UIntToGlyphEncodingInfoMap::value_type(
                    itGlyphs->mGlyphCode, GlyphEncodingInfo(*itEncoded, itGlyphs->mUnicodeValues)));
        }
    ...
    for (; itList != inGlyphsList.end() && encodingResult.first; ++itList)
    {
        ...
        for (; it != itList->end() && encodingResult.first; ++it)
        {
            ...
            else if (0x2022 == it->mUnicodeValues.front())
            {
                ...
                // now i don't know if it's related or not...but acrobat isn't happy when i'm using winansi with bullet.
                // and text coming after that bullet may be corrupted. so i'm forcing CID if i hit bullet till i know
                // better.
                encodingResult.first = false;
            }
            else
