...
static const uint8_t scNewLine[2] = {'\r', '\n'};
...
{
    mStreamForWriting->Write((const uint8_t *)inKeyword.c_str(), inKeyword.size());
    EndLine();
}

static const uint8_t scSlash[1] = {'/'};
...
void PrimitiveObjectsWriter::WriteName(const std::string &inName, ETokenSeparator inSeparate)
{
    ...
    This syntax is required to represent any of the delimiter or white-space characters or the number sign character
    itself; it is recommended but not required for characters whose codes are outside the range 33 (!) to 126 (~).
    */
...
void PrimitiveObjectsWriter::WriteInteger(long long inIntegerToken, ETokenSeparator inSeparate)
{
    ...
    mStreamForWriting->Write((const uint8_t *)buffer, strlen(buffer));
    WriteTokenSeparator(inSeparate);
}

static const uint8_t scLeftParanthesis[1] = {'('};
static const uint8_t scRightParanthesis[1] = {')'};
...
void PrimitiveObjectsWriter::WriteUnsafeLiteralString(const std::string &inString, ETokenSeparator inSeparate)
{
    ...
    mStreamForWriting->Write((const uint8_t *)inString.c_str(), inString.size());
...
void PrimitiveObjectsWriter::WriteDouble(double inDoubleToken, ETokenSeparator inSeparate)
{
    ...
    size_t sizeToWrite = DetermineDoubleTrimmedLength(result);

    mStreamForWriting->Write((const uint8_t *)(result.c_str()), sizeToWrite);
    WriteTokenSeparator(inSeparate);
}
...
static const uint8_t scTrue[4] = {'t', 'r', 'u', 'e'};
static const uint8_t scFalse[5] = {'f', 'a', 'l', 's', 'e'};
...
static const uint8_t scNull[4] = {'n', 'u', 'l', 'l'};
...
static const uint8_t scLeftAngle[1] = {'<'};
static const uint8_t scRightAngle[1] = {'>'};
