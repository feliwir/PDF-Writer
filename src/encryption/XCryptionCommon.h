...
class XCryptionCommon
{
  public:
    ...
    /*
        ...
        2. Call Initial Setup with the PDF level. return bool will tell you if XCryption is supported for this input. if
       not...don't bother with the rest
        3. Call SetupInitialEncryptionKey. This may require some initial computations with other algorithms of this
       object, per what input you may possess. It is fine to call any method prior to SetupInitialEncryptionKey, but
       those that relate to state - OnObjectStart,OnObjectEnd, GetCurrentObjectKey. these require the computation. Call
       this whenever you acquired the required input for it, no matter when.
    ...
    */
    ...
    // PDF xcryption algorithms (Important! length must be passed in bytes and not in bits. which normally means - the
    // PDF value / 8)
  ...
  private:
    ...
    ByteList ComputeEncryptionKeyForObject(ObjectIDType inObjectNumber,
                                           unsigned long inGenerationNumber); // with algorithm3_1
};