...
/*
    ...
    Optionally Implement RenewStreamConnection, which is called before any operator is written, in order to make sure
   there's a stream there to write to
...
class AbstractContentContext
{
  public:
    ...
    struct GraphicOptions
    {
        ...
        GraphicOptions(EDrawingType inDrawingType = eStroke, EColorSpace inColorSpace = eRGB,
                       unsigned long inColorValue = 0, double inStrokeWidth = 1.0, bool inClose = false)
        {
    ...
    struct TextOptions
    {
        ...
        TextOptions(PDFUsedFont *inFont, double inFontSize = 1, EColorSpace inColorSpace = eRGB,
    ...
    // High level methods
    void DrawRectangle(double inLeft, double inBottom, double inWidth, double inHeight,
                       const GraphicOptions &inOptions = GraphicOptions());
    void DrawSquare(double inLeft, double inBottom, double inEdge, const GraphicOptions &inOptions = GraphicOptions());
    void DrawCircle(double inCenterX, double inCenterY, double inRadius,
                    const GraphicOptions &inOptions = GraphicOptions());
    ...
    void DrawImage(double inX, double inY, const std::string &inImagePath,
                   const ImageOptions &inOptions = ImageOptions());

    // PDF Operators. For explanations on the meanings of each operator read Appendix A "Operator Summary" of the PDF
    ...
    void DoubleQuoteLow(double inWordSpacing, double inCharacterSpacing,
                        const std::string &inText); // matches the operator "
    ...
    PrimitiveObjectsWriter &GetPrimitiveWriter()
    {
        return mPrimitiveWriter;
    }
  ...
  private:
    ...
    // Derived classes should optionally use this method if the stream needs updating (use calls to SetPDFStreamForWrite
    // for this purpose)
    ...
    virtual void ScheduleImageWrite(const std::string &inImagePath, unsigned long inImageIndex, ObjectIDType inObjectID,
                                    const PDFParsingOptions &inParsingOptions) = 0;
