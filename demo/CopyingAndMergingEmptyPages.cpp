...
EStatusCode CopyingAndMergingEmptyPages::PrepareSampleEmptyPDF(const TestConfiguration &inTestConfiguration,
                                                               const string &inEmptyFileName)
{
    PDFWriter pdfWriter;
    EStatusCode status;
    ...
    do
    {
        ...
        status = pdfWriter.StartPDF(RelativeURLToLocalPath(inTestConfiguration.mSampleFileBase, inEmptyFileName),
                                    ePDFVersion13);
...
PDFHummus::EStatusCode CopyingAndMergingEmptyPages::CreateFormFromEmptyPage(
    const TestConfiguration &inTestConfiguration, const string &inEmptyFileName)
{
    EStatusCode status;
    PDFWriter pdfWriter;
    ...
    do
    {
        status = pdfWriter.StartPDF(
            RelativeURLToLocalPath(inTestConfiguration.mSampleFileBase, "CreateFormFromEmptyPage.pdf"), ePDFVersion13);
        if (status != PDFHummus::eSuccess)
        {
            cout << "failed to start PDF\n";
            break;
        ...
        EStatusCodeAndObjectIDTypeList result = pdfWriter.CreateFormXObjectsFromPDF(
            RelativeURLToLocalPath(inTestConfiguration.mSampleFileBase, inEmptyFileName), PDFPageRange(),
            ePDFPageBoxMediaBox);
...
PDFHummus::EStatusCode CopyingAndMergingEmptyPages::CreatePageFromEmptyPage(
    const TestConfiguration &inTestConfiguration, const string &inEmptyFileName)
{
    EStatusCode status;
    ...
    do
    {
        ...
        status = pdfWriter.StartPDF(
            RelativeURLToLocalPath(inTestConfiguration.mSampleFileBase, "CreatePageFromEmptyPage.pdf"), ePDFVersion13);
        if (status != PDFHummus::eSuccess)
        {
            cout << "failed to start PDF\n";
            break;
...
PDFHummus::EStatusCode CopyingAndMergingEmptyPages::MergeEmptyPageToPage(const TestConfiguration &inTestConfiguration,
                                                                         const string &inEmptyFileName)
{
    PDFWriter pdfWriter;
    EStatusCode status;
    ...
    {
        status = pdfWriter.StartPDF(
            RelativeURLToLocalPath(inTestConfiguration.mSampleFileBase, "MergeEmptyPageToPage.pdf"), ePDFVersion13);
        if (status != PDFHummus::eSuccess)
            break;
        ...
        status = pdfWriter.MergePDFPagesToPage(
            page, RelativeURLToLocalPath(inTestConfiguration.mSampleFileBase, inEmptyFileName), singlePageRange);
        if (status != PDFHummus::eSuccess)
            break;
...
PDFHummus::EStatusCode CopyingAndMergingEmptyPages::MergeEmptyPageToForm(const TestConfiguration &inTestConfiguration,
                                                                         const string &inEmptyFileName)
{
    PDFWriter pdfWriter;
    EStatusCode status;
    ...
    {
        status = pdfWriter.StartPDF(
            RelativeURLToLocalPath(inTestConfiguration.mSampleFileBase, "MergeEmptyPageToForm.pdf"), ePDFVersion13);
        if (status != PDFHummus::eSuccess)
            break;
        ...
        copyingContext = pdfWriter.CreatePDFCopyingContext(
            RelativeURLToLocalPath(inTestConfiguration.mSampleFileBase, inEmptyFileName));
        if (status != PDFHummus::eSuccess)
            break;
