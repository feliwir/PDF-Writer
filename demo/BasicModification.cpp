    ...
    do
    {
        status = pdfWriter.ModifyPDF(
            ...
            LogConfiguration(true, true,
                             RelativeURLToLocalPath(inTestConfiguration.mSampleFileBase,
                                                    string("Modified") + inSourceFileName + string(".log"))));
    ...
    do
    {
        // first copy source file to target
        {
            ...
            status = targetFile.OpenFile(RelativeURLToLocalPath(
                inTestConfiguration.mSampleFileBase, string("InPlaceModified") + inSourceFileName + string(".pdf")));
            if (status != eSuccess)
            {
        ...
        status = pdfWriter.ModifyPDF(
            RelativeURLToLocalPath(inTestConfiguration.mSampleFileBase,
                                   string("InPlaceModified") + inSourceFileName + string(".pdf")),
            ePDFVersion13, "",
            LogConfiguration(true, true,
                             RelativeURLToLocalPath(inTestConfiguration.mSampleFileBase,
                                                    string("InPlaceModified") + inSourceFileName + string(".log"))));
        if (status != PDFHummus::eSuccess)
        {
            cout << "failed to start PDF\n";
            break;
