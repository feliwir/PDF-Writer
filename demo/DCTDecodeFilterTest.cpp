...
EStatusCode DCTDecodeFilterTest::Run(const TestConfiguration &inTestConfiguration)
{
    ...
    do
    {
        ...
        status = pdfWriter.ModifyPDF(
            RelativeURLToLocalPath(inTestConfiguration.mSampleFileBase, testFileName), ePDFVersion13,
            RelativeURLToLocalPath(inTestConfiguration.mSampleFileBase, string("mod") + testFileName));
                                                        ...
                                                        const string &inTestFileName)
{
    ...
    do
    {
        status = pdfWriter.StartPDF(RelativeURLToLocalPath(inTestConfiguration.mSampleFileBase, inTestFileName),
                                    ePDFVersion13);
        if (status != PDFHummus::eSuccess)
        {
