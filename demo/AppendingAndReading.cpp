...
EStatusCode AppendingAndReading::Run(const TestConfiguration &inTestConfiguration)
{
    ...
    do
    {
        // write initial file
        {
            ...
            status =
                outputFile.OpenFile(RelativeURLToLocalPath(inTestConfiguration.mSampleFileBase, "ReadAndWrite.txt"));
            if (status != eSuccess)
            {
        ...
        status = appendToFile.OpenFile(RelativeURLToLocalPath(inTestConfiguration.mSampleFileBase, "ReadAndWrite.txt"),
                                       true);
        if (status != eSuccess)
        {
