...
EStatusCode MergePDFPages::Run(const TestConfiguration &inTestConfiguration)
{
    ...
    */
    ...
    {
        cout << "Failed [TestSuffixGraphicsMerge()]: test for merging of one page with some graphics after the merged "
                "page\n";
        status = PDFHummus::eFailure;
    }
    ...
    {
        cout << "Failed [TestBothGraphicsMerge()]: test for merging of one page with some graphics before and after "
                "the merged page\n";
        status = PDFHummus::eFailure;
    }
    ...
    {
        cout << "Failed [MergeTwoPageInSeparatePhases()]: test for merging of two pages in two separate pheases with "
                "some graphics between them\n";
        status = PDFHummus::eFailure;
    }

    if (MergeTwoPageWithEvents(inTestConfiguration) != PDFHummus::eSuccess)
    {
        cout << "Failed [MergeTwoPageWithEvents()]: test for merging of two pages with some graphics between them, "
    ...
    {
        cout << "Failed [MergeTwoPageUsingCopyingContext()]: test for merging of multiple pages with some graphics "
                "between them and also using some as xobjects, using the copying contexts\n";
        status = PDFHummus::eFailure;
    }
    return status;
}
...
EStatusCode MergePDFPages::TestOnlyMerge(const TestConfiguration &inTestConfiguration)
{
    ...
    do
    {
        status = pdfWriter.StartPDF(RelativeURLToLocalPath(inTestConfiguration.mSampleFileBase, "TestOnlyMerge.pdf"),
                                    ePDFVersion13);
        ...
        status = pdfWriter.MergePDFPagesToPage(
            page, RelativeURLToLocalPath(inTestConfiguration.mSampleFileBase, "TestMaterials/BasicTIFFImagesTest.pdf"),
            singePageRange);
        if (status != PDFHummus::eSuccess)
            break;
...
EStatusCode MergePDFPages::TestPrefixGraphicsMerge(const TestConfiguration &inTestConfiguration)
{
    ...
    do
    {
        status = pdfWriter.StartPDF(
            RelativeURLToLocalPath(inTestConfiguration.mSampleFileBase, "TestPrefixGraphicsMerge.pdf"), ePDFVersion13);
        if (status != PDFHummus::eSuccess)
            break;
...
EStatusCode MergePDFPages::TestSuffixGraphicsMerge(const TestConfiguration &inTestConfiguration)
{
    ...
    {
        status = pdfWriter.StartPDF(
            RelativeURLToLocalPath(inTestConfiguration.mSampleFileBase, "TestSuffixGraphicsMerge.pdf"), ePDFVersion13);
        if (status != PDFHummus::eSuccess)
            break;
...
EStatusCode MergePDFPages::TestBothGraphicsMerge(const TestConfiguration &inTestConfiguration)
{
    ...
    {
        status = pdfWriter.StartPDF(
            RelativeURLToLocalPath(inTestConfiguration.mSampleFileBase, "TestBothGraphicsMerge.pdf"), ePDFVersion13);
        if (status != PDFHummus::eSuccess)
            break;
...
EStatusCode MergePDFPages::MergeTwoPageInSeparatePhases(const TestConfiguration &inTestConfiguration)
{
    ...
    {
        status = pdfWriter.StartPDF(
            RelativeURLToLocalPath(inTestConfiguration.mSampleFileBase, "MergeTwoPageInSeparatePhases.pdf"),
            ePDFVersion13);
        if (status != PDFHummus::eSuccess)
            break;
...
EStatusCode MergePDFPages::MergeTwoPageWithEvents(const TestConfiguration &inTestConfiguration)
{
    ...
    {
        status = pdfWriter.StartPDF(
            RelativeURLToLocalPath(inTestConfiguration.mSampleFileBase, "MergeTwoPageWithEvents.pdf"), ePDFVersion13);
        if (status != PDFHummus::eSuccess)
            break;
...
EStatusCode MergePDFPages::MergePagesUsingCopyingContext(const TestConfiguration &inTestConfiguration)
{
    ...
    {
        status = pdfWriter.StartPDF(
            RelativeURLToLocalPath(inTestConfiguration.mSampleFileBase, "MergePagesUsingCopyingContext.pdf"),
            ePDFVersion13);
        if (status != PDFHummus::eSuccess)
            break;
