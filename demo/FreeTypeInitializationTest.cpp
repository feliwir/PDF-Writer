...
EStatusCode FreeTypeInitializationTest::Run(const TestConfiguration &inTestConfiguration)
{
    Singleton<Trace>::GetInstance()->SetLogSettings(
        RelativeURLToLocalPath(inTestConfiguration.mSampleFileBase, "FreeTypeTest.txt"), true, true);
...
EStatusCode FreeTypeInitializationTest::ShowFaceProperties(FreeTypeWrapper &inFreeType, const string &inFontFilePath,
...
EStatusCode FreeTypeInitializationTest::ShowGlobalFontProperties(FreeTypeWrapper &inFreeType, FT_Face inFace)
{
    ...
    cout << "Font BBox = [" << face->bbox.xMin << " " << face->bbox.yMin << " " << face->bbox.xMax << " "
         << face->bbox.yMax << "]\n";
